...
function isExecException(err: unknown): err is IExecException {
  return (
    (err as IExecException).stdout !== undefined &&
...
async function createCertificatePfx() {
  ...
  const certificateFileName =
    env["TEMP"] +
    `\\code-sign-certificate-${Math.floor(Math.random() * 1000000)}.pfx`;
...
async function removeCertificatePfx(certificateFileName: string | null) {
  if (certificateFileName === null) return;
...
async function signWithSigntool(
  ...
  try {
    const { stdout } = await asyncExec(
      `"${signtool}" sign /f ${certificateFileName} /p ${certificateFilePassword} /tr ${timestampUrl} /td sha256 /fd sha256 "${fileName}"`
    );
    console.log(stdout);
    return true;
  } catch (err: unknown) {
...
async function signNupkg(certificateFileName: string, fileName: string) {
  ...
  try {
    const { stdout } = await asyncExec(
      `"${nugetFileName}" sign "${fileName}" -CertificatePath ${certificateFileName} -Timestamper ${timestampUrl}`
    );
    console.log(stdout);
    return true;
  } catch (err: unknown) {
...
async function trySignFile(
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (signtoolFileExtensions.includes(extension)) {
      ...
      if (await signNupkg(certificateFileName, fileName)) return;
    }
  }
...
async function run() {
  ...
  try {
    ...
    const certificateFilePassword = core.getInput("password");
    if (certificateFileName !== null)
      await signFiles(certificateFileName, certificateFilePassword);
